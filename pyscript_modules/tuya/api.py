...
class TuyaAPI:
  ...
  def _handle(self, result):
    if not isinstance(result, dict):
      logger.error("Unexpected response from Tuya API: %s", result)
      raise ValueError("Invalid result, check logs")
    if result.get("success"):
      return result.get("result")
    elif result.get("errorCode") == "USER_SESSION_INVALID":
      raise InvalidUserSession
    elif result.get("errorCode") == "USER_PASSWD_WRONG":
      raise InvalidAuthentication
    else:
      logger.error("Error! Code: %s, message: %s, result: %s", result.get("errorCode"), result.get("errorMsg"), result)
      raise ValueError("Invalid result, check logs")
  ...
  def groups(self):
    # Tuya liefert bei einem Account ohne angelegte Gruppen/Locations "null"
    # statt einer leeren Liste zurueck. Ohne "or []" wuerde die aufrufende
    # for-Schleife mit 'NoneType' object is not iterable abstuerzen.
    return self._api({"action": "tuya.m.location.list"}) or []

  def devices(self, group_id):
    devs = []
    # Gleiche Absicherung: eine leere Gruppe liefert von der Tuya-API "null".
    for dev in self._api({"action": "tuya.m.my.group.device.list", "gid": group_id}) or []:
  ...
  def get_dps(self, device_id, dps=None):
    result = self._api({"action": "tuya.m.device.dp.get"}, {"devId": device_id}) or {}
    if dps is not None:
  ...
  def ir_children(self, parent_id):
    subdevs = []
    # Auch hier: kann "null" statt [] sein, wenn keine IR-Unterknoten existieren.
    devices = self._api({"action": "tuya.m.device.sub.list"}, {"meshId": parent_id}) or []
  ...
  def ir_get_buttons(self, gateway_id, device_id):
    record = self._api({"action": "tuya.m.infrared.record.get"}, {"devId": device_id, "gwId": device_id, "subDevId": device_id, "vender": IR_VENDOR}) or {}
  ...
  def _ir_learned_buttons(self, gateway_id, device_id):
    # Kann "null" sein, wenn fuer dieses Geraet keine eingelernten Tasten existieren.
    buttons = self._api(
